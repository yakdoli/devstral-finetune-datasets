...
from ..context7_connector import HybridSearcher, Context7Client, DocumentRetriever
...
class ConversationConfig:
    ...
    context7_max_results: int = 5
    use_context7_in_prompt: bool = True
...
class ConversationGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            config: 대화 생성 설정
            hybrid_searcher: 하이브리드 검색기 (Context7 통합용)
        """
        self.client = client
        ...
        self.context7_enabled = config.enable_context7
    ...
    async def _generate_single_conversation(
        ...
        try:
            # Context7에서 관련 정보 검색
            context7_context = ""
            if self.context7_enabled and self.hybrid_searcher:
                try:
                    # 문서의 제목을 기반으로 Context7 검색
                    query = document.get('title', document.get('component', ''))
                    if query:
                        search_results = await self.hybrid_searcher.search(query, max_results=self.config.context7_max_results)
                        
                        # 검색 결과에서 상위 N개의 정보 추출
                        if search_results:
                            context7_context = "\n\n관련 정보:\n"
                            for i, result in enumerate(search_results[:3]):  # 상위 3개만 사용
                                context7_context += f"{i+1}. {result.title}\n"
                                context7_context += f"   출처: {result.source}\n"
                                context7_context += f"   내용: {result.content[:200]}...\n"
                                context7_context += f"   관련성 점수: {result.hybrid_score:.2f}\n\n"
                            
                            logger.info(f"Context7에서 {len(search_results)}개의 관련 정보 찾음")
                except Exception as e:
                    logger.warning(f"Context7 검색 실패: {e}")
                    context7_context = ""
            
            # 프롬프트 생성 - Context7 정보 포함
            user_question = f"Please provide detailed information about {document.get('title', 'document')}."
            
            # Context7 정보가 있으면 프롬프트에 추가
            if context7_context and self.config.use_context7_in_prompt:
            ...
            # 대화 구조화
            conversation = self._structure_conversation(document, messages, response, context7_context)
    ...
    def _structure_conversation(
        ...
        # Context7 정보 추가
        if context7_context:
            metadata["context7_enabled"] = True
            metadata["context7_context_length"] = len(context7_context)
            metadata["context7_sources"] = []
            
            # Context7 출처 정보 추출
            import re
            source_pattern = r"출처: ([^\n]+)"
            sources = re.findall(source_pattern, context7_context)
            metadata["context7_sources"] = sources
            
            logger.info(f"Context7 정보 메타데이터에 추가: {len(sources)}개 출처")
        else:
            metadata["context7_enabled"] = False
