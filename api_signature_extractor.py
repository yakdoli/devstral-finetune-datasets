...
class APISignatureExtractor:
    ...
    def __init__(self, context7_library_id: str = "/syncfusion/syncfusion"):
    ...
    def extract_api_from_code(self, code: str, component: str) -> List[APISignature]:
        ...
        for method in methods:
            ...
            if method_match:
                ...
                # 리턴 타입 추출
                return_type = "void"
                return_match = re.search(f'{method}\\s*\\([^)]*\\)\\s*->\\s*([^{]+)', code)
                if return_match:
    ...
    async def extract_from_context7(self, component_name: str) -> List[APISignature]:
        ...
        try:
            # Context7 라이브러리 ID 확인
            if not self.context7_library_id:
                library_id = await resolve_library_id("syncfusion")
                if library_id:
                    self.context7_library_id = library_id
                else:
                    logger.error("Could not resolve Context7 library ID for Syncfusion")
                    return []
            ...
            # API 정보 추출
            signatures = []
            if docs and 'code_snippets' in docs:
                for snippet in docs['code_snippets']:
                    if component_name.lower() in snippet.get('title', '').lower():
    ...
    async def extract_all_apis(self, md_staging_path: Path) -> Dict[str, ComponentInfo]:
        """모든 컴포넌트에서 API 정보 추출"""
        # md_staging에서 기본 추출
            ...
            if context7_signatures:
                ...
                for sig in context7_signatures:
                    if sig.name not in existing_names:
                        components[component_name].api_signatures.append(sig)
...
async def main():
    ...
    # API 정보 추출
    components = await extractor.extract_all_apis(md_staging_path)
